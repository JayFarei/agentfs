...
export function buildPlannedChainProcedure(args: {
  name: string;
  tenantId: string;
  description: string;
  sourceTrajectoryId: string;
  ...
  return {
    ...
    createdAt: new Date().toISOString(),
    matcher: {
      intent: "planned_chain",
      examples: [args.questionExample],
      questionFingerprint: args.questionFingerprint
    },
    params: {
      filename: args.filename
    },
    implementation: {
      kind: "planned_chain",
      plan: args.plan,
      learnedFunctionNames: args.learnedFunctionNames,
      learnedAgentNames: args.learnedAgentNames,
      questionFingerprint: args.questionFingerprint
    }
  };
}
...
export function buildProcedureFromTrajectory(trajectory: TrajectoryRecord): StoredProcedure {
  if (trajectory.calls.some((call) => call.primitive.startsWith("planner."))) {
    throw new Error(
      "Planner trajectories crystallise inline via runPlannedQuery — they must not be endorsed through buildProcedureFromTrajectory."
    );
  }
...
function renderProcedureTs(procedure: StoredProcedure): string {
  if (procedure.implementation.kind === "planned_chain") {
    const fnNames = procedure.implementation.learnedFunctionNames;
    const agentNames = procedure.implementation.learnedAgentNames;
    return `// Generated by local AtlasFS crystallization (planned_chain).
// Source trajectory: ${procedure.sourceTrajectoryId}
//
// This chain was emitted by the observer's planTrajectory call. It binds the
// question shape to an ordered ExecutionPlan whose steps reference both
// boot-time primitives and these learned primitives:
${fnNames.map((n) => `//   - learned function: ${n}`).join("\n") || "//   (no learned functions)"}
${agentNames.map((n) => `//   - learned agent:    ${n}`).join("\n") || "//   (no learned agents)"}
//
// Replay is dispatched by the runtime: matchProcedure (matcher.ts) hits the
// questionFingerprint, runQuery sees implementation.kind === "planned_chain",
// and the planner executor walks plan.steps using the saved learned
// primitives. This .ts file is cosmetic — the JSON next to it is the
