...
existing interface, draft the trajectory, then execute the committed
trajectory. Plan runs are scratch work; execute runs are the auditable
answer path. The runtime records the execute trajectory; if it is validated
and reusable, the observer learns a parameterised `/lib/<tenant>/<name>.ts`
interface for next time.
...
- `$DATAFETCH_HOME/sessions/<sessionId>/plan/attempts/<trajectoryId>/` — non-learnable plan artifacts.
...
datafetch plan -e '<source>'        # exploratory run; cannot be learned from
datafetch plan <file>               # exploratory saved trajectory

datafetch execute -e '<source>'     # committed run; can be learned from
...
the next agent — and it doesn't feed the system that learns new interfaces.
...
### Tier 2 — Learned interfaces

Typed interfaces the observer has learned from validated trajectories. They
...
(with a *"Use when..."* clause). If an interface's description matches the
...
not re-compose the chain by hand; the interface already encodes it.
...
- **Trajectories you replay** strengthen the case for learning.
  When the observer sees a validated reusable shape, it can publish a
  learned interface.
- **Interfaces you call** get exercised. If their shape converges with a new
  pattern, the observer can improve or supersede them.
...
sample, inspect available interfaces, write helper code, and draft the trajectory.
A run may be broad, but it is not the final answer and it cannot be learned from.
...
--- envelope ---
{
  ...
  "functionName": "<name-of-learned-interface-if-any>",
...
`df.lib.*` helper or learned interface, infer the table plan with
