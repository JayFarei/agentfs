...
The single execute snippet must be the minimal reusable chain in one run: db retrieval -> `df.lib.pickFiling` -> `df.lib.inferTableMathPlan` -> `df.lib.executeTableMath` -> final print. Resolve fallbacks and candidate-search uncertainty in plan mode before execute; do not put fallback branches, manual candidate scoring, or alternate query attempts inside execute. The execute artifact should read like a reusable typed interface, not a debug script.
