...
The single execute snippet must contain the full workflow in one run: db retrieval, filing selection or explicit fallback choice, table-plan inference, table math, and final print. It may contain fallback branches inside the one snippet, but it must not be a bare search that only prints candidates. Prefer the reusable chain `df.db.*` -> `df.lib.pickFiling` or an explicit candidate choice -> `df.lib.inferTableMathPlan` -> `df.lib.executeTableMath`; do not compute the answer only with ad hoc local JavaScript after search.
