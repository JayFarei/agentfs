...
- **R7.** A two-question demo on FinQA proves both properties simultaneously. Q1 (e.g. table-math on chemicals revenue range) returns `mode: "novel"`, tier 4, non-zero `cost.tokens.cold`, and ≥1 `llmCalls`. Q2 (the same intent shape on coal) returns `mode: "interpreted"`, tier ≤ 2, `cost.tokens.cold` = 0, and `llmCalls` = 0. The function called by Q2 is the file written by the observer after Q1.
...
7. Headline demo (R7 in full): `pnpm datafetch demo` runs Q1 and Q2; envelopes show the documented mode/tier/cost/llmCalls diff; both answers are correct against fixtures.
